...
export type ProcessedPlayer = {
  ...
  // ── Promedio ponderado ──
  // Para el promedio del equipo. Difiere de derivedMinutesPlayed solo en expulsados:
  //   jugador normal    → impactMinutes = derivedMinutesPlayed
...
export function processMatch(match: Match): ProcessedMatch {
  ...
  function buildProcessedPlayer(p: Player): ProcessedPlayer {
    ...
    // impactMinutes: igual a derivedMinutesPlayed excepto para expulsados.
    // Para un expulsado, el slot queda vacío hasta el final → penaliza al equipo.
    let impactMinutes: number
    if (status === 'sub_never_entered' || status === 'sub_expelled_before_entry') {
      impactMinutes = 0
    } else if (s.redCard) {
      // Expulsado: desde que entró hasta el FINAL del partido
      const entryMinute = p.starter ? 0 : (s.minuteIn ?? 0)
      impactMinutes = computePeriodAwareMinutes(entryMinute, matchEnd)
    } else {
      impactMinutes = derivedMinutesPlayed
    }
